...
func TestDiscoverResolvers_DesignTokensFieldPriority(t *testing.T) {
	...
	mfs.AddFile("/project/package.json", `{
		"name": "test",
		"dependencies": { "@both/pkg": "^1.0.0" }
	}`, 0644)
	mfs.AddFile("/project/node_modules/@both/pkg/package.json", `{
		"name": "@both/pkg",
		"designTokens": { "resolver": "field.resolver.json" },
		"exports": {
			".": { "designTokens": "./export.resolver.json" }
		}
	}`, 0644)
	mfs.AddFile("/project/node_modules/@both/pkg/field.resolver.json", `{"version":"2025.10"}`, 0644)
	mfs.AddFile("/project/node_modules/@both/pkg/export.resolver.json", `{"version":"2025.10"}`, 0644)
