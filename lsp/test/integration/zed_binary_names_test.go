...
func TestZedBinaryNamingConsistency(t *testing.T) {
	...
	zedSource, err := os.ReadFile("../../../extensions/zed/src/asimonim.rs")
	...
	// Ensure binary name uses asimonim prefix
	assert.Contains(t, sourceStr, `"asimonim-{}-{}{}"`,
		"Zed extension should use asimonim binary name format")

	// Ensure GitHub release points to asimonim repo
	assert.Contains(t, sourceStr, `"bennypowers/asimonim"`,
		"Zed extension should reference asimonim GitHub repo")

	// Ensure lsp subcommand is passed
	assert.Contains(t, sourceStr, `"lsp"`,
		"Zed extension should pass lsp subcommand")
