...
func BenchmarkParse_WithPositions(b *testing.B) {
	...
	for _, size := range sizes {
		b.Run(fmt.Sprintf("tokens=%d", size), func(b *testing.B) {
			...
			opts := parser.Options{
				...
				SkipPositions: false, // Full LSP mode
			}
...
func BenchmarkParse_SkipPositions(b *testing.B) {
	sizes := []int{100, 500, 1000, 5000}
